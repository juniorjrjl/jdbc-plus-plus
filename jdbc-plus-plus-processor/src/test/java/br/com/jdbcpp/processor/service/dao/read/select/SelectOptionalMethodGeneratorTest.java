...
import br.com.jdbcpp.processor.service.dao.read.select.param.SelectOptionalMethodGeneratorArgumentProvider;
...
@Fixture(
        resourcePath = "service/dao/read/select/SelectOptionalMethodGeneratorTest.txt",
        packageName = "com.example"
)
class SelectOptionalMethodGeneratorTest {
    ...
    @ParameterizedTest
    @ArgumentsSource(SelectOptionalMethodGeneratorArgumentProvider.class)
    void shouldBuildSelectOptionalMethodWithBasicConfiguration(final String returnTypeStr,
        ...
        final var packException = useCustomException
                ...
                : sqlException;
        final var generator = createSelectOptionalMethodGenerator(sqlException);

        final var strategies = createStrategies(strategyType);
        
        final var methodInfo = createSelectOptionalMethodBuilder(
                returnTypeStr,
    ...
    private static final List<Arguments> shouldUseInstanceForSelectOptionalMethodInfo =
            List.of(
                    Arguments.of(SelectNullableMethodInfo.class, false),
                    Arguments.of(SelectOptionalMethodInfo.class, true),
    ...
    @ParameterizedTest
    @FieldSource
    void shouldUseInstanceForSelectOptionalMethodInfo(final Class<? extends MethodInfo> methodInfoClass,
                                                      final boolean expectedResult) {
        final var sqlException = processingEnv.getElementUtils()
                .getTypeElement(SQLException.class.getCanonicalName()).asType();
        final var generator = createSelectOptionalMethodGenerator(sqlException);

        final var selectOptionalMethodInfo = mock(methodInfoClass);
        assertThat(generator.useInstance(selectOptionalMethodInfo)).isEqualTo(expectedResult);
    }

    private SelectOptionalMethodGenerator createSelectOptionalMethodGenerator(final TypeMirror sqlException) {
        return new SelectOptionalMethodGenerator(selectResultSetDelegator, statementBuilder, sqlException);
    }
    ...
    private SelectOptionalMethodInfo.SelectOptionalMethodInfoBuilder createSelectOptionalMethodBuilder(
        ...
        final var optionalType = processingEnv.getElementUtils()
        ...
        return MethodInfo.builder()
                ...
                .<SelectOptionalMethodInfo.SelectOptionalMethodInfoBuilder>asReadType(QueryType.OPTIONAL)
                .withStrategies(strategies)
                .withStrategyType(strategyType)
                .withContainerReturnTypeMirror(optionalType);
    }
